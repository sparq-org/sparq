...
def find_open_issue(shard: str) -> str | None:
    ...
    `[differential-fuzz] shard=equality` (the shards are differential.yml's generator
    categories) carries exactly the punctuation gh's search tokeniser handles
    unreliably, and the search index also lags — either would MISS an issue this filer
    already opened and mint a duplicate.
    """
...
def file_github_issue(bead_id: str, shard: str, parsed: dict, args) -> None:
    ...
    gh_dedupe.ensure_label(
        LABEL, color="b60205",
        description="differential-fuzz mismatch vs Oxigraph (candidate wrong-answer bug)",
        log=log,
    )
