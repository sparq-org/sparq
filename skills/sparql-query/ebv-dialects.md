...
when an integration rewrites algebra. Bare `From<Query>`/`into_query` carry
...
read a global or thread-local selector. This query announcement API does not
change UPDATE parsing or claim complete update-dialect support.
...
The W3C runner selects the pinned draft for `sparql12/` query suites and REC
