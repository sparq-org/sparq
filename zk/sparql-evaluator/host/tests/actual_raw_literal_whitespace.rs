...
#[test]
fn actual_guest_raw_literal_whitespace() {
    let r0vm = std::env::var_os("RISC0_SERVER_PATH")
        .map(PathBuf::from)
        .expect("real local r0vm executable is required");
    let executor = ExternalProver::new("real-sparq-raw-literal-whitespace", r0vm);
    ...
    for case in corpus["cases"].as_array().unwrap() {
        ...
        eprintln!("actual raw literal guest case {}", case["id"]);
