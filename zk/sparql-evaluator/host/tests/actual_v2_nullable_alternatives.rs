...
#[test]
fn actual_v2_guest_nullable_alternatives_and_capacity_controls() {
    let r0vm = std::env::var_os("RISC0_SERVER_PATH")
        .map(PathBuf::from)
        .expect("real local r0vm executable is required");
    let executor = ExternalProver::new("real-sparq-v2-nullable-alternatives", r0vm);
    ...
    for case in std::iter::once(&original).chain(corpus["cases"].as_array().unwrap()) {
        ...
        eprintln!("actual V2 nullable alternative guest case {}", case["id"]);
