...
mod v2_checks {
    ...
    use sparq_proved_evaluator_model::v2::{
        Dialect, Policy, PrivateDataset, Request, VERSION, Witness, admit, dataset_commitment,
        evaluate,
    };
    fn v2_witness(case: &serde_json::Value) -> Witness {
        let dataset = PrivateDataset {
            nquads: case["dataset_ntriples"].as_str().unwrap_or_default().into(),
    ...
    #[test]
    fn v2_numeric_capacity_is_rejected_by_the_evaluation_relation() {
        let corpus: serde_json::Value = serde_json::from_str(include_str!(
            "../../fixtures/conformance/numeric-capacity.json"
        ))
        .unwrap();
    ...
    #[test]
    fn v2_strict_profile_retains_normative_builtin_goldens_and_rejects_capacity_controls() {
        let corpus: serde_json::Value = serde_json::from_str(include_str!(
            "../../../../crates/sparq-engine/tests/fixtures/builtin_edges.json"
        ))
        .unwrap();
